...
\section{实验现象}
\begin{itemize}
    \item 对于初值条件1，newton法的CG收敛很慢！原因是正定矩阵条件数相比admm的CG太大：admm的CG有$\rho$进行调节
...
\par 结果记录；\begin{itemize}
\item 随机初值，取$dt=1e10$，$Nx=Ny=1000$，计算一个时间步，并行化程序用16个计算核心：
\begin{table}
    \centering
    \begin{tabular}{c|c|c|c|c|c}
    算法&判断收敛阈值&cpu\_time(s)&主迭代次数(次)& 能量 &残量\\
    \hline
    admm & 1e-6 & 87.211 & admm:43 & 1.2331\\
    \hline
    admm\_parallel & 1e-6 & 10.834 & admm:44 & 1.2331 & 0.00148471\\
    \hline
    newton & 1e-8 & 321.924 & newton:1633 & 1.2331\\
    \hline
    newton & 1e-7 & 100.303 & newton:407 & 1.2331\\
    \hline
    newton\_parallel & 1e-8 & 28.991 & newton:1477 & 1.2331 & 2.49905e-05\\
    \hline
    newton\_parallel & 1e-7 & 10.788 & newton:389 & 1.2331 & 0.000249798\\
    \end{tabular}
    \caption{随机初值数值结果}
    \label{随机初值数值结果}
\end{table}
\item 光滑初值，取$dt=1e10$，$Nx=Ny=1000$，计算一个时间步，并行化程序用16个计算核心：注意当阈值取$1e-8$时能量下降才一致
\begin{table}
    \centering
    \begin{tabular}{c|c|c|c|c|c}
    算法&判断收敛阈值&cpu\_time(s)&主迭代次数(次)& 能量 & 残量\\
    \hline
    admm & 1e-6& 113.87 & admm:65 & 0.595444\\
    \hline
    admm\_parallel & 1e-6 & 12.81 & admm:65 & 0.595444 & 0.00209806\\
    \hline
    newton &  1e-8 &  153.212 & newton:8 & 0.59545\\
    \hline
    newton &  1e-7 &  153.316 & newton:8 & 0.59545\\
    \hline
    newton\_parallel & 1e-8 & 12.149 & newton:8 & 0.59545 & 122.292\\
    \hline
    newton\_parallel & 1e-7 & 12.979 & newton:7 & 0.59545 & 124.567
    \end{tabular}
    \caption{光滑初值数值结果}
    \label{光滑初值数值结果}
\end{table}
